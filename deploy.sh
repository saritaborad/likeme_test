...
if ssh $deploy_host "[ -d $uploads_folder ]"; then
  # If it exists, move it to a temporary location
  ssh $deploy_host "mv $uploads_folder $deploy_path/uploads_temp"
fi
...
ssh $deploy_host "cd $deploy_path/backend && npm install && pm2 restart likeme_test"

# Navigate to the frontend directory on the server
ssh $deploy_host "cd $deploy_path/frontend && npm install && npm run build"
...
if ssh $deploy_host "[ -d $deploy_path/uploads_temp ]"; then
  # If it was moved, restore it to its original location
  ssh $deploy_host "mv $deploy_path/uploads_temp $uploads_folder"
fi
